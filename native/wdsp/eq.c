/*  PORTED from ramdor/Thetis (Project Files/Source/wdsp/eq.c) into the
...
EQP create_eqp (int run, int size, int nc, int mp, double *in, double *out, int nfreqs, double* F, double* G, int ctfmode, int wintype, int samplerate)
{
	...
	// FORK: the engine's fircore takes a partition factor; 4 is what
	// its own eq.c passed. Partitioning changes how the convolution is
	// chunked, not the filter, so this is sound-equivalent to Thetis.
...
/********************************************************************************************************
*																										*
*		FORK (trininfo/station-engine): equalizer response, for plotting								*
*																										*
********************************************************************************************************/

/*  The engine binds GetTXAEQDraw / GetRXAEQDraw (X, Y), which the NURBS
    equalizer this file replaced used to provide. Thetis's equalizer has no
    such getter, so these are re-implemented here -- and not as a redrawing
    of the control points: they call the SAME eq_impulse, with the SAME
    arguments and scale SetTXAEQProfile uses, then measure the magnitude
    response of that filter. The impulse cache returns the identical
    coefficients the running stage was built from, so the curve is the
    filter being applied, not a picture of the sliders.

    CONTRACT (unchanged from the NURBS version the engine was written
    against): X and Y must each hold EQ_DRAW_POINTS doubles. X is Hz,
    log-spaced from 20 Hz to min(20 kHz, fs/2); Y is dB, 0 dB = unity.

    THREADING: Thetis's SetXXAEQProfile frees and reallocates F/G/Q without
    taking a lock -- safe in Thetis, where one UI thread makes every call.
    These getters read the same arrays, so the ENGINE must serialize profile
    writes and draws; WdspDspEngine routes both through one lock. Nothing
    here adds a lock, to keep the ported setters exactly as Thetis wrote
    them.
*/

#define EQ_DRAW_POINTS 1024

static void eqp_draw (EQP a, double* X, double* Y)
{
	const double scale = 1.0 / (2.0 * a->size);
	double* imp = eq_impulse (a->nc, a->nfreqs, a->F, a->G, a->Q, a->samplerate,
		scale, a->ctfmode, a->wintype);
	double fmin = 20.0;
	double fmax = 0.5 * a->samplerate;
	if (fmax > 20000.0) fmax = 20000.0;
	double lr = log (fmax / fmin);
	int k, n;
	for (k = 0; k < EQ_DRAW_POINTS; k++)
	{
		double f = fmin * exp (lr * (double)k / (double)(EQ_DRAW_POINTS - 1));
		double w = 2.0 * PI * f / a->samplerate;
		// H(w) = sum h[n] e^{-jwn}, with e^{-jwn} advanced by a rotating
		// phasor rather than a cos/sin per tap: 16384 taps x 1024 points is
		// otherwise ~34M transcendental calls per draw.
		double cr = cos (w), ci = -sin (w);
		double zr = 1.0, zi = 0.0;
		double re = 0.0, im = 0.0;
		for (n = 0; n < a->nc; n++)
		{
			double hr = imp[2 * n + 0], hi = imp[2 * n + 1];
			re += hr * zr - hi * zi;
			im += hr * zi + hi * zr;
			double t = zr * cr - zi * ci;
			zi = zr * ci + zi * cr;
			zr = t;
			// Renormalise now and then; the phasor's magnitude would
			// otherwise drift measurably over 16k multiplies.
			if ((n & 1023) == 1023)
			{
		...
		X[k] = f;
		Y[k] = 20.0 * log10 (mag > 1.0e-12 ? mag : 1.0e-12);
	}
	_aligned_free (imp);
}
...
{
	eqp_draw (txa[channel].eqp.p, X, Y);
}

PORT
void GetRXAEQDraw (int channel, double* X, double* Y)
{
	eqp_draw (rxa[channel].eqp.p, X, Y);
}
